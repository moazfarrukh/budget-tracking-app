...
export const userLogIn = (req: Request, res: Response, next: NextFunction) => {
    if (typeof req.user !== "undefined") {
        ...
        User.findById(requestUser._id).then((user: any) => {
...
export const userRefreshToken = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (typeof refreshToken !== 'undefined') {
        try {
            ...
            User.findById(user_id).then((user: any) => {

                if (user) {
                    const tokenIndex = user.refreshToken.findIndex((item: any) => item.refreshToken === refreshToken)
                    if (tokenIndex === -1) {
                        res.statusCode = 401;
...
export const userLogOut = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.user !== undefined) {
        User.findById(reqUser._id).then(
            (user: any) => {
